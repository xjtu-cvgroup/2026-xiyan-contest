...
| `04-short-gate-edges.start.json` | S14 入边缩短至 4 帧内 | 富资源对手可瞬拆时走得分；动态堵仅在能确定拒止交付时接管 |
...
`3.98.26` 保留上述 48 帧资源窗口。曾尝试在无固定 S10 的图上收紧为
12 帧，实测会把 `03/06` 对纯冲锋手从胜局翻成败局，因此不予采用；
本轮只收紧动态堵的执行证明，不改变成熟的开局经济路线。

`3.98.20` 接入三张图的真实回放后补齐路线契约：`03` 的固定处理动作不再
被宫门影子竞速覆盖，`04` 完成 S02 后不再从 S03 折返重复换乘，`05` 会
识别纯冰鉴 S02 是 88 帧高代价支线并直接走快线。S02 决策按资源分档：
快/短马可接受最多 48 帧完整到门溢价，普通资源最多 12 帧，因此保留
`03/04` 的快马收益而跳过 `05`。三图定向矩阵合计 18/18 胜。

`3.98.27` 完成规则反证审计：04 短门图仅允许能确定阻止交付的动态卡，
普通任务/资源只在真实 DRAW 后错峰；对手承诺宫门而我方没有完整设卡先手
时立即跟速，双方到门后禁止回头。最终矩阵为 6/6 拓扑通过、48/48 对局
获胜，零非法动作、零策略异常。
